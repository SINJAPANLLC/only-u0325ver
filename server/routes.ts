...
export async function registerRoutes(
  ...
  app.get("/api/creator/sales", isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Get subscription sales directly from subscriptions table
      const subscriptionSales = await db
        .select({
          id: subscriptions.id,
          planId: subscriptions.planId,
          status: subscriptions.status,
          createdAt: subscriptions.expiresAt, // Use expiresAt to determine payment date
      ...
      const recentSubscriptionSales = subscriptionSales.map(sub => ({
        id: sub.id,
        productName: `サブスク: ${sub.planName || "プラン"}`,
        amount: sub.planPrice || 0,
      ...
      // Get subscription earnings (from subscriptions table)
      const subscriptionTotal = subscriptionSales.reduce((sum, sub) => sum + (sub.planPrice || 0), 0);
...
以下のJSON形式で返してください：
{
  ...
  (async () => {
    try {
      ...
      // Seed automated email templates
      const autoEmailTemplates = [
        {
          name: "パスワードリセット",
          subject: "【Only-U】パスワードのリセット",
          type: "automated",
          htmlContent: `<!DOCTYPE html><html lang="ja"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head><body style="margin:0;padding:0;background:#fff0f5;font-family:-apple-system,BlinkMacSystemFont,'Hiragino Sans',sans-serif;"><table width="100%" cellpadding="0" cellspacing="0" style="background:#fff0f5;padding:32px 16px;"><tr><td align="center"><table width="520" cellpadding="0" cellspacing="0" style="max-width:520px;width:100%;background:#fff;border-radius:24px;overflow:hidden;border:1px solid #fce4ec;box-shadow:0 4px 24px rgba(236,64,122,.08);"><tr><td style="background:linear-gradient(135deg,#f48fb1,#f06292,#ec407a);padding:32px;text-align:center;"><div style="display:inline-block;background:rgba(255,255,255,.2);border-radius:50px;padding:6px 20px;"><span style="font-size:18px;font-weight:900;color:#fff;letter-spacing:3px;">Only-U 💕</span></div></td></tr><tr><td style="padding:40px;text-align:center;"><div style="font-size:48px;margin-bottom:16px;">🔑</div><h1 style="margin:0 0 12px;font-size:22px;font-weight:800;color:#c2185b;">パスワードのリセット</h1><p style="margin:0 0 28px;font-size:14px;color:#ad1457;line-height:1.8;opacity:.8;">パスワードリセットのリクエストを受け付けました。✨<br>下のボタンから新しいパスワードを設定してください。</p><a href="{{reset_url}}" style="display:inline-block;background:linear-gradient(135deg,#f06292,#ec407a);color:#fff;text-decoration:none;font-size:14px;font-weight:800;padding:14px 40px;border-radius:50px;box-shadow:0 4px 12px rgba(236,64,122,.3);">パスワードをリセット ✨</a><p style="margin:24px 0 0;font-size:12px;color:#f48fb1;">⏰ このリンクは1時間後に無効になります</p></td></tr><tr><td style="background:#fff0f5;padding:20px;text-align:center;border-top:1px solid #fce4ec;"><p style="margin:0;font-size:11px;color:#f48fb1;">© 2025 Only-U 💗 | 合同会社SIN JAPAN KANAGAWA</p></td></tr></table></td></tr></table></body></html>`,
        },
        {
          name: "お問い合わせ受付",
          subject: "【Only-U】お問い合わせを受け付けました",
          type: "automated",
          htmlContent: `<!DOCTYPE html><html lang="ja"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head><body style="margin:0;padding:0;background:#fff0f5;font-family:-apple-system,BlinkMacSystemFont,'Hiragino Sans',sans-serif;"><table width="100%" cellpadding="0" cellspacing="0" style="background:#fff0f5;padding:24px;"><tr><td align="center"><table width="520" cellpadding="0" cellspacing="0" style="max-width:520px;background:#fff;border-radius:24px;overflow:hidden;border:1px solid #fce4ec;box-shadow:0 4px 24px rgba(236,64,122,.08);"><tr><td style="background:linear-gradient(135deg,#f48fb1,#f06292,#ec407a);padding:24px;text-align:center;"><span style="font-size:16px;font-weight:900;color:#fff;letter-spacing:2px;">Only-U 📩 お問い合わせ</span></td></tr><tr><td style="padding:32px;"><div style="background:#fff0f5;border-radius:12px;padding:16px;border:1px solid #fce4ec;margin-bottom:16px;"><p style="margin:0 0 8px;font-size:13px;">件名: {{subject}}</p><p style="margin:0;font-size:13px;">カテゴリ: {{category}}</p></div><p style="margin:0;font-size:14px;color:#666;line-height:1.8;">お問い合わせを受け付けました。担当者が確認の上、2〜3営業日以内にご返信いたします。</p></td></tr><tr><td style="background:#fff0f5;padding:16px;text-align:center;border-top:1px solid #fce4ec;"><p style="margin:0;font-size:11px;color:#f48fb1;">© 2025 Only-U 💗 | 合同会社SIN JAPAN KANAGAWA</p></td></tr></table></td></tr></table></body></html>`,
        },
        {
          name: "会員登録完了通知",
          subject: "【Only-U】ご登録ありがとうございます！",
          type: "automated",
          htmlContent: `<!DOCTYPE html><html lang="ja"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head><body style="margin:0;padding:0;background:#fff0f5;font-family:-apple-system,BlinkMacSystemFont,'Hiragino Sans',sans-serif;"><table width="100%" cellpadding="0" cellspacing="0" style="background:#fff0f5;padding:32px 16px;"><tr><td align="center"><table width="520" cellpadding="0" cellspacing="0" style="max-width:520px;width:100%;background:#fff;border-radius:24px;overflow:hidden;border:1px solid #fce4ec;box-shadow:0 4px 24px rgba(236,64,122,.08);"><tr><td style="background:linear-gradient(135deg,#f48fb1,#f06292,#ec407a);padding:32px;text-align:center;"><div style="display:inline-block;background:rgba(255,255,255,.2);border-radius:50px;padding:6px 20px;"><span style="font-size:18px;font-weight:900;color:#fff;letter-spacing:3px;">Only-U 💕</span></div></td></tr><tr><td style="padding:40px;text-align:center;"><div style="font-size:56px;margin-bottom:16px;">🎉</div><h1 style="margin:0 0 12px;font-size:22px;font-weight:800;color:#c2185b;">ご登録ありがとうございます！</h1><p style="margin:0 0 8px;font-size:15px;color:#ad1457;font-weight:700;">{{display_name}} さん、ようこそ！</p><p style="margin:0 0 28px;font-size:14px;color:#ad1457;line-height:1.8;opacity:.8;">Only-Uへの登録が完了しました。✨<br>お気に入りのクリエイターを探して、<br>特別なコンテンツを楽しみましょう！</p><a href="https://only-u.fun" style="display:inline-block;background:linear-gradient(135deg,#f06292,#ec407a);color:#fff;text-decoration:none;font-size:14px;font-weight:800;padding:14px 40px;border-radius:50px;box-shadow:0 4px 12px rgba(236,64,122,.3);">Only-Uを始める ✨</a></td></tr><tr><td style="padding:0 40px 32px;"><table width="100%" cellpadding="0" cellspacing="0"><tr><td width="33%" style="text-align:center;padding:16px 8px;background:#fff0f5;border-radius:12px 0 0 12px;"><div style="font-size:24px;margin-bottom:6px;">🎬</div><p style="margin:0;font-size:11px;font-weight:700;color:#c2185b;">限定動画</p></td><td width="33%" style="text-align:center;padding:16px 8px;background:#fce4ec;border-left:2px solid #fff;border-right:2px solid #fff;"><div style="font-size:24px;margin-bottom:6px;">📡</div><p style="margin:0;font-size:11px;font-weight:700;color:#c2185b;">LIVE配信</p></td><td width="33%" style="text-align:center;padding:16px 8px;background:#fff0f5;border-radius:0 12px 12px 0;"><div style="font-size:24px;margin-bottom:6px;">💎</div><p style="margin:0;font-size:11px;font-weight:700;color:#c2185b;">限定特典</p></td></tr></table></td></tr><tr><td style="background:#fff0f5;padding:20px;text-align:center;border-top:1px solid #fce4ec;"><p style="margin:0;font-size:11px;color:#f48fb1;">© 2025 Only-U 💗 | 合同会社SIN JAPAN KANAGAWA</p></td></tr></table></td></tr></table></body></html>`,
        },
        {
          name: "クリエイター承認通知",
          subject: "【Only-U】クリエイター申請が承認されました！",
          type: "automated",
          htmlContent: `<!DOCTYPE html><html lang="ja"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head><body style="margin:0;padding:0;background:#fff0f5;font-family:-apple-system,BlinkMacSystemFont,'Hiragino Sans',sans-serif;"><table width="100%" cellpadding="0" cellspacing="0" style="background:#fff0f5;padding:32px 16px;"><tr><td align="center"><table width="520" cellpadding="0" cellspacing="0" style="max-width:520px;width:100%;background:#fff;border-radius:24px;overflow:hidden;border:1px solid #fce4ec;box-shadow:0 4px 24px rgba(236,64,122,.08);"><tr><td style="background:linear-gradient(135deg,#f48fb1,#f06292,#ec407a);padding:32px;text-align:center;"><div style="display:inline-block;background:rgba(255,255,255,.2);border-radius:50px;padding:6px 20px;"><span style="font-size:18px;font-weight:900;color:#fff;letter-spacing:3px;">Only-U 🌟 Creator</span></div></td></tr><tr><td style="padding:40px;text-align:center;"><div style="font-size:56px;margin-bottom:16px;">⭐</div><h1 style="margin:0 0 12px;font-size:22px;font-weight:800;color:#c2185b;">クリエイター承認おめでとうございます！</h1><p style="margin:0 0 8px;font-size:15px;color:#ad1457;font-weight:700;">{{display_name}} さん</p><p style="margin:0 0 28px;font-size:14px;color:#ad1457;line-height:1.8;opacity:.8;">クリエイター申請が承認されました！✨<br>今すぐ動画・ライブ配信・商品販売など<br>クリエイター機能をお使いいただけます。</p><a href="https://only-u.fun/creator/dashboard" style="display:inline-block;background:linear-gradient(135deg,#f06292,#ec407a);color:#fff;text-decoration:none;font-size:14px;font-weight:800;padding:14px 40px;border-radius:50px;box-shadow:0 4px 12px rgba(236,64,122,.3);">クリエイターダッシュボードへ ✨</a></td></tr><tr><td style="padding:0 40px 32px;"><div style="background:#fff0f5;border-radius:12px;padding:20px;border:1px solid #fce4ec;"><p style="margin:0 0 12px;font-size:13px;font-weight:700;color:#c2185b;">🎬 使えるようになった機能</p><p style="margin:0 0 8px;font-size:13px;color:#555;">✅ 動画・写真のアップロード・有料設定</p><p style="margin:0 0 8px;font-size:13px;color:#555;">✅ LIVE配信（ライブコマース対応）</p><p style="margin:0 0 8px;font-size:13px;color:#555;">✅ デジタル・物販商品の販売</p><p style="margin:0;font-size:13px;color:#555;">✅ サブスクリプションプランの設定</p></div></td></tr><tr><td style="background:#fff0f5;padding:20px;text-align:center;border-top:1px solid #fce4ec;"><p style="margin:0;font-size:11px;color:#f48fb1;">© 2025 Only-U 💗 | 合同会社SIN JAPAN KANAGAWA</p></td></tr></table></td></tr></table></body></html>`,
        },
        {
          name: "サブスクリプション開始通知",
          subject: "【Only-U】サブスクリプションが開始されました",
          type: "automated",
          htmlContent: `<!DOCTYPE html><html lang="ja"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head><body style="margin:0;padding:0;background:#fff0f5;font-family:-apple-system,BlinkMacSystemFont,'Hiragino Sans',sans-serif;"><table width="100%" cellpadding="0" cellspacing="0" style="background:#fff0f5;padding:32px 16px;"><tr><td align="center"><table width="520" cellpadding="0" cellspacing="0" style="max-width:520px;width:100%;background:#fff;border-radius:24px;overflow:hidden;border:1px solid #fce4ec;box-shadow:0 4px 24px rgba(236,64,122,.08);"><tr><td style="background:linear-gradient(135deg,#f48fb1,#f06292,#ec407a);padding:32px;text-align:center;"><div style="display:inline-block;background:rgba(255,255,255,.2);border-radius:50px;padding:6px 20px;"><span style="font-size:18px;font-weight:900;color:#fff;letter-spacing:3px;">Only-U 💕</span></div></td></tr><tr><td style="padding:40px;text-align:center;"><div style="font-size:56px;margin-bottom:16px;">💖</div><h1 style="margin:0 0 12px;font-size:22px;font-weight:800;color:#c2185b;">サブスクリプション開始！</h1><p style="margin:0 0 8px;font-size:15px;color:#ad1457;font-weight:700;">{{creator_name}} さんをサブスクしました</p><p style="margin:0 0 28px;font-size:14px;color:#ad1457;line-height:1.8;opacity:.8;">サブスクリプションが開始されました。✨<br>限定コンテンツをお楽しみください！</p><div style="background:#fff0f5;border-radius:12px;padding:16px;border:1px solid #fce4ec;margin-bottom:24px;text-align:left;"><p style="margin:0 0 6px;font-size:13px;color:#666;">プラン: <strong style="color:#c2185b;">{{plan_name}}</strong></p><p style="margin:0 0 6px;font-size:13px;color:#666;">料金: <strong style="color:#c2185b;">{{price}}pt / 月</strong></p><p style="margin:0;font-size:13px;color:#666;">次回更新: <strong>{{next_billing_date}}</strong></p></div><a href="https://only-u.fun" style="display:inline-block;background:linear-gradient(135deg,#f06292,#ec407a);color:#fff;text-decoration:none;font-size:14px;font-weight:800;padding:14px 40px;border-radius:50px;box-shadow:0 4px 12px rgba(236,64,122,.3);">コンテンツを見る ✨</a></td></tr><tr><td style="background:#fff0f5;padding:20px;text-align:center;border-top:1px solid #fce4ec;"><p style="margin:0;font-size:11px;color:#f48fb1;">© 2025 Only-U 💗 | 合同会社SIN JAPAN KANAGAWA</p></td></tr></table></td></tr></table></body></html>`,
        },
      ];

      for (const tmpl of autoEmailTemplates) {
        const [existing] = await db.select().from(emailTemplates).where(
          eq(emailTemplates.name, tmpl.name)
        );
        if (!existing) {
          await db.insert(emailTemplates).values(tmpl);
          console.log(`Seeded auto email template: ${tmpl.name}`);
        }
      }
